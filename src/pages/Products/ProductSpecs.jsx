...
const ProductSpecs = () => {
    ...
    useEffect(() => {
        fetch('https://brand-shop-server-ruby.vercel.app/products')
            .then(res => res.json())
    ...
    return (
        <div className='container mx-auto my-10'>
                <h2 href='/shop' className='mb-3 mx-6 lg:mx-0 uppercase font-semibold text-xs bg-[#002B44] text-[#F9FAFB] p-1 w-fit rounded-md'>shop{pathname}</h2>
            <div className='flex flex-col lg:flex-row gap-10 items-start justify-items-center px-6 lg:px-0'>
                <a href="#" className='lg:w-1/2 group relative block overflow-hidden'>
                    <button
                    ...
                    <img
                        src={image}
                        alt=""
                        className="w-full h-[55vh] object-cover transition duration-500 group-hover:scale-105 rounded-md"
                    />
                </a>
                <div className='lg:w-1/2'>
                    ...
                    <h1 className='my-2 capitalize text-[#002B44] text-4xl font-semibold'>{name}</h1>
                    <h4 className='mb-2 capitalize text-[#002B44] text-sm font-normal w-[90%]'>description : {short_description}</h4>
                    <p className="my-4 capitalize text-[#002B44] text-xl font-semibold">Price : ${price}</p>
                    <div className='flex items-center gap-2 my-4'>
                        <p className='text-[#002B44] capitalize text-sm font-semibold'>reviews : {rating}</p>
                        <img className='w-4' src="https://i.ibb.co/c2wM3sV/star.png" alt="" />
                    </div>
                    <p className="my-4 capitalize text-[#002B44] font-semibold">sold : {sold} item</p>
                    <p className='mb-y capitalize text-[#002B44] text-sm font-normal w-[90%]'>{details}</p>
                </div>
